...
export function BkashResultView() {
  ...
  if (status === "success") {
    return (
      <main className="mx-auto max-w-lg p-6 pb-16 text-center">
        ...
        <h1 className="mb-1 text-xl font-semibold">Payment successful!</h1>
        <p className="mb-6 text-sm text-muted">
          {order ? (
            <>
              Order #{order._id.slice(-6).toUpperCase()} &mdash; {formatCurrency(order.totalAmount)}, paid via
              bKash.
            </>
          ) : (
            "Your bKash payment went through."
          )}
        </p>
  ...
  const message =
    status === "cancelled"
      ? "You cancelled the bKash payment — nothing was charged, and your cart is still here."
  ...
  return (
    <main className="mx-auto max-w-lg p-6 pb-16 text-center">
      ...
      <h1 className="mb-1 text-xl font-semibold">Payment not completed</h1>
